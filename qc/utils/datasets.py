...
class ReplayBuffer(Dataset):
    """Replay buffer class with success/failure label support."""
    ...
    def create_from_initial_dataset(cls, init_dataset, size):
        ...
        # === THÊM FIELD is_success cho offline data (mặc định = 0) ===
    ...
    def clear(self):
        """Clear the replay buffer. (GIỮ NGUYÊN như code gốc)"""
        self.size = self.pointer = 0

    # === HÀM MỚI: sample batch có nhãn success/failure ===
    def sample_with_success_label(self, batch_size):
        """Sample a batch and return is_success label."""
        idxs = self.get_random_idxs(batch_size)
        batch = self.get_subset(idxs)
        batch['is_success'] = self['is_success'][idxs]
        return batch
